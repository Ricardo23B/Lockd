...
### Corregido
- **Scanner sin resultados inventados (frente 3):** `check_firewall` reportaba falso "inactivo" sin root (`ufw status` exige privilegios) — ahora lee `/etc/ufw/ufw.conf` y, sin datos, responde `unknown`, nunca inventa. Checks SSH: una máquina sin servidor OpenSSH es "secure" (sin exposición), no una recomendación de endurecer lo inexistente. `check_proc_hidepid` y `check_apparmor` devuelven `unknown` ante fallos de lectura/permisos en vez de opinar. `check_suid_binaries` alineado a la denylist del módulo (un Ubuntu estándar ya no es "insecure" por fusermount3/polkit-agent-helper-1); un test verifica que ambas denylists no se desincronicen. `tests/test_scanner_accuracy.py` cubre los cinco caminos.
- **Packaging pip funcional (frente 4):** `pip install .` fallaba — `build-backend` inválido (`setuptools.backends.legacy:build` → `setuptools.build_meta`), el paquete se instalaba con el nombre `src`, y el entry point apuntaba a un módulo no incluido. Renombrado el paquete `src/` → `lockd/` (todos los imports actualizados), el punto de entrada vive en `lockd/entry.py` (console script `lockd = lockd.entry:main`) y `lockd.py` queda como shim de desarrollo. Verificado en venv limpio: instala como `lockd`, el comando funciona, sin paquete `src` espurio.
- **Coherencia editorial (frente 5):** README clona desde GitLab (no GitHub); árbol del README actualizado al nuevo layout; eliminados 4 archivos muertos (`legacy_executor.py`, `module_registry.py` y los shims `state_manager.py`/`profile_manager.py` que nadie importaba); `[tool.flake8]` muerto fuera de pyproject (la config real es `.flake8`); comentario NOTE duplicado eliminado; `MODULE_MODULE_CATEGORIES` (definida y jamás usada) renombrada a `KNOWN_CATEGORIES` y conectada: una categoría desconocida en `modules.yaml` ahora genera warning en vez de pasar en silencio.
- **CI: shellcheck bloqueante:** se eliminaron los `|| true` que ignoraban el lint de los scripts — la parte más peligrosa del proyecto ahora tiene lint que rompe el build. Corregidos los 3 hallazgos pendientes en `_common.sh` (directiva de shell, SC2155, SC2034).
- **El estado mostrado podía mentir (frente 2 / v1.0.1):** `lockd status` y la GUI leían solo `state.json` (por usuario); cambios hechos a mano, por otro usuario o por scripts fallidos eran invisibles. Los 16 check scripts —que existían sin usarse— quedan conectados: el Controller reconcilia el estado con el sistema real al arrancar (`refresh_states()`), y tras cada operación contrasta el resultado con el check del módulo (`_verified()`) — un script que reporta éxito sin efecto real deja el módulo en `error`. Contrato formalizado de exit codes (0 activo / 1 inactivo / 2 no determinable, documentado en `docs/CREACION_MODULO.md`): un check que no puede determinar sin privilegios conserva lo registrado en vez de inventar.
- `enable_firewall/check.sh` reportaba falso "inactivo" sin root (`ufw status` requiere privilegios): ahora lee `/etc/ufw/ufw.conf` con fallback a `systemctl is-active`.
- `restrict_suid_binaries/check.sh` reportaba falso "inactivo" cuando `/var/lib/lockd/backups` (0750) no era legible: ahora distingue "nunca usado" (1) de "sin permisos para determinar" (2).
- `tests/test_state_reconciliation.py`: 25 tests — mapeo de exit codes, reconciliación al arrancar, post-verificación de operaciones, y contrato global de los 16 checks reales (existen, sintaxis válida, rc∈{0,1,2}, sin root, sin efectos sobre el sistema, <10s).
